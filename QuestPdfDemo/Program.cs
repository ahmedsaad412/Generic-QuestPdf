...
{
    new Header("ID", "رقم الهوية", x => ((Product)x).ID, 1,1, type: HeaderType.Primitive),
    new Header("Product Name", "اسم السلعة", x => ((Product)x).ProductName, 2, type: HeaderType.Primitive),
    new Header("Price", "السعر", x => ((Product)x).Price, 4, width: 1, type: HeaderType.Primitive),
...
var products = new List<Product>
{
    ...
    new Product { ID = 2, ProductName = "Product B", Price = 149.50m,Types = new List<MyType>{new MyType { Id = 1, Value = "Type1" },new MyType { Id = 2, Value = "Type2" } }, Colors = new List<string> { "Green", "Yellow","REd"}} ,
...
var myreport = report.GeneratePdf(reportOptions);
Console.WriteLine();
